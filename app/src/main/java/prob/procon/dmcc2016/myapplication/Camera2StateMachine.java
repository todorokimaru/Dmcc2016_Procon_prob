...
public class Camera2StateMachine {
    ...
    private final State mOpenCameraState = new State("OpenCamera") {
        public void enter() throws CameraAccessException {
            ...
            boolean camera_pos = false;
            try{
                for(String camera_num : mCameraManager.getCameraIdList()) {
                    CameraCharacteristics check_characteristics = mCameraManager.getCameraCharacteristics(camera_num);
                    if(check_characteristics.get(CameraCharacteristics.LENS_FACING) == CameraCharacteristics.LENS_FACING_BACK){
                        Log.v("Camera2","Facing:BACK");
                        cameraId = Camera2Util.getCameraId(mCameraManager, CameraCharacteristics.LENS_FACING_BACK);
                        camera_pos = true;
                    }
                    if(check_characteristics.get(CameraCharacteristics.LENS_FACING) == CameraCharacteristics.LENS_FACING_BACK){
                        Log.v("Camera2","Facing:BACK");
                        if(camera_pos == false)
                            cameraId = Camera2Util.getCameraId(mCameraManager, CameraCharacteristics.LENS_FACING_FRONT);
                    }
                }
            }
            catch (CameraAccessException e){
